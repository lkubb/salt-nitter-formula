...
We will see a quick setup to configure the ``nitter`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nitter`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``nitter`` formulas under ``/srv/salt/nitter/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nitter/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nitter/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nitter/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nitter/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nitter`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``nitter`` formulas under ``/srv/salt/nitter/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nitter/parameters/roles
...
We create a configuration for the role ``nitter/server`` in ``/srv/salt/nitter/parameters/roles/nitter/server.yaml``:
...
We create another configuration for the role ``nitter/client`` in ``/srv/salt/nitter/parameters/roles/nitter/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nitter/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nitter/client
...
.. code-block:: console
    ...
    mv /srv/salt/nitter/parameters/dns:domain/  '/srv/salt/nitter/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nitter-as-json:
      test.nop:
        - name: {{ nitter | json }}
...
Here is an example based on `template-formula/nitter/config/file.sls`_:
    ...
    nitter-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='nitter-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ nitter.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``nitter`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ nitter['winner'] }}
...
.. _template-formula/nitter/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nitter/config/file.sls
